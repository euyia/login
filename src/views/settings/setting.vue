...
<script>
import settingUiApi from '@/api/settingUi'
export default {
  name: 'Setting',
  data() {
    return {
  ...
  methods: {
    getGroupSettingDefinitions() {
      settingUiApi.getRoupSettingDefinitions().then(setting => {
    ...
    updateSettingValues(formName, index) {
      ...
      settingUiApi.postEtSettingValues(obj).then((res) => {
        console.log(res)
        this.$notify({
